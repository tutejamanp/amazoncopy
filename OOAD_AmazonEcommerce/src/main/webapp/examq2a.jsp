...
$.get(api , function(data, status){
    ...
    while(currentCount < data.length) {
    	if(currentCount % 4 == 0) {
    		...
    		items = items + "<figcaption class='info-wrap'><a href='#' class='title'> item id: "+data[currentCount].productid+"</a>";
    		items = items + "<div><a href='#' class='title'> product name:"+data[currentCount].productname+"</a></div>";
    		items = items + "<div><a href='#' class='title'> seller id: "+data[currentCount].seller+"</a></div>";
    		items = items + "<div><a href='#' class='title'> number ratings : "+data[currentCount].totalreviews+"</a></div>";
...
function loadListings2(){
	   
	$('#dynamicSearch2').empty();
	var data = [];
    	    $.ajax(
	        		{
	        			type : 'GET',
	        			...
	        			success : function(result){
        					for (var i in result){
	        					...
	        					console.log("total reviews are :"+result[i].totalreviews);
	        					console.log("nRatings are :"+$("#Nratings").val());
	        					
	        					if(result[i].discountedprice <  $("#MinPrice").val() || result[i].discountedprice > $("#MaxPrice").val() || result[i].totalreviews <
	        						$("#Nratings").val()
		...
		<ul class="list-inline">
		  <li class="list-inline-item">
		  	<div class="form-inline">
		  		<label class="mr-2">Enter Price From </label>
...
<div class="row">
	<div class="col-md-21-24"> 
		<ul class="list-inline">
		  <li class="list-inline-item">
		  	<div class="form-inline">
		  		<label class="mr-2">Enter Number of Rating </label>
				<input id="Nratings" class="form-control form-control-sm" placeholder="Max" type="number" value="0">
				<button id="applyFilter2" type="submit" class="btn btn-sm ml-2" onclick="loadListings2();"> Submit </button>
			</div>
		  </li>
		</ul>
	</div> <!-- col.// -->
