...
var api;
var currentCount = 0;
var items = "<div class='row' data-jplist-group='group1'  >";
api = "http://localhost:8055/amazon.com/webapi/ProductController/searchproducts/"+localStorage.getItem("basedSearch");
$.get(api , function(data, status){
    ...
    while(currentCount < data.length) {
    		...
    		items = items + "<div class='col-sm-12'><div class='row' style='margin-bottom:-12px;'><div class='col-sm-12' data-jplist-item><figure class='card card-product' onclick='viewProductDetails("+data[currentCount].id+");''>";
    		
    		//inserting image
    		items = items + "<div class='row'><div class='col-sm-6'><div class='img-wrap' style='height:100px;width:100px;'><img src='"+data[currentCount].product_images[0].url+"'  style='height:95px;'/></div></div>";
    		
    		items = items + "<div class='col-sm-6'><figcaption class='info-wrap'><a href='#' class='title'>"+data[currentCount].productname+"</a>";
    		...
    		items = items + "</figure></div></div></div></div></div>";
    		currentCount++;	
    }
    	    ...
    	    $.ajax(
	        		{
	        			...
	        			url : "http://localhost:8055/amazon.com/webapi/ProductController/searchproducts/"+localStorage.getItem("basedSearch"),
	        			async : false,
 ...
 */	        							|| ($("#temp1_valuedropdown option:selected").text().toLowerCase() != result[i].temp1_value.toLowerCase() && $("#temp1_valuedropdown option:selected").text().toLowerCase() != "none" )
	        				...
	        				//alert(data[0].id);
	        				var currentCount = 0;
	        				var items = "<div class='row' data-jplist-group='group1' style='margin-bottom:0px;' >";
	        				$('#count').html(data.length+" results found.")
	        			    while(currentCount < data.length) {
        ...
        $.ajax(
        		{
        			type : 'GET',
        			contentType : 'application/json',
        			url : api = "http://localhost:8055/amazon.com/webapi/ProductController/searchproducts/"+localStorage.getItem("basedSearch"),
    			dataType : "json", // data type of response
    			success : function(result){
    				//alert("hello sort");
...
function GetSortOrderDescending(prop) { 
    return function(a, b) {  
		...
		function displayListings(result)
		{
			...
			data+="<div class='col-sm-12'><a href='item.jsp?id="+result.id+
			"'> <div style=''>"+
...
<!-- 		  <label>Color:</label><li class="list-inline-item dropdown"><a href="#" id="Color"></a></li> -->
		  ...
		  <li class="list-inline-item">
		  	<div class="form-inline">
				...
				<button id="applyFilter" type="submit" class="btn btn-sm ml-2" onclick="loadListings();">Ok</button><span class="px-2"> - </span>
