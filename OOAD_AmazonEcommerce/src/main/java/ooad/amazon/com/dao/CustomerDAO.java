...
public static List<Product> getuserbyname (String name) {
	
	Session ses = CommonSessionFactory.sf.openSession();
	Query query = ses.createQuery("from Customer where fname = "+"'"+name+"'");
	List<Customer> addrList  = (List<Customer>) query.list(); 
	for (Customer c : addrList) {
...
System.out.println(addrList.toString());
		return addrList.get(0).getWishlist();
} 
